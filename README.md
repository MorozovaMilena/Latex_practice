...
![](https://github.com/MorozovaMilena/Latex_practice/blob/main/ImagesForMdFile/Пример1.png)
![](https://github.com/MorozovaMilena/Latex_practice/blob/main/ImagesForMdFile/Пример1(1).png)


![](https://github.com/MorozovaMilena/Latex_practice/blob/main/ImagesForMdFile/Пример2.png)
![](https://github.com/MorozovaMilena/Latex_practice/blob/main/ImagesForMdFile/Пример2(1).png)

## Изображения
Время от времени необходимо добавлять изображения в документы. При использовании LaTeX все изображения будут проиндексированы автоматически и помечены последовательными номерами при использовании среды фигур и пакета **graphicx**.
...
\begin{document}
\begin{figure}
  \includegraphics[width=\linewidth]{Названиефайла.расширение}
...
Среда рисунков обеспечивает нумерацию и позиционирование изображения в документе. Чтобы добавить фигуру, необходимо использовать команду **\includegraphics**. В качестве параметра в скобках берется ширина изображения и путь к файлу изображения. **\linewidth** в скобках означает, что изображение будет масштабировано в соответствии с шириной документа. В результате меньшие изображения увеличиваются, а большие изображения уменьшаются соответственно. 

В следующей команде мы задаем **\caption**, который является текстом, показанным под изображением, и **\label**, который является невидимым, но полезным, если мы хотим сослаться на наш рисунок в нашем документе. Можно использовать команду **\ref** для ссылки на рисунок (помеченный меткой) в тексте, после чего он будет заменен правильным номером. 
...
```
* **[h]** -  указывает текущее местоположение, что означает, что изображение будет помещено в текущую позицию, но если места на этой странице недостаточно для размещения изображения, изображение перейдет на следующую страницу;
* **[t]** - картинка помещается вверху страницы;
* **[b]** - картинка помещается внизу страницы;
* **[p]** - система автоматически установит положение изображения;

### Множественные изображения
Среда **subfigure** позволяет размещать несколько изображений в определенном месте рядом друг с другом, и использование довольно простое.
```sh
\begin{figure}
