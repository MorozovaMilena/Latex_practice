...
```sh
\multicolumn{3}{|c|}{Название заголовка}\\
\cline{промежуток, например 1-3}
```
Команда **\multicolumn** имеет три обязательных аргумента: первый аргумент указывает количество столбцов, по которым распространяется заголовок; второй аргумент указывает положение заголовка **(l,c,r)** ; а третий аргумент - текст заголовка. Команда **\cline{1-3}** указывает начальный столбец (здесь, 1) и конечный столбец (здесь, 3), над которым должна быть нарисована строка.
